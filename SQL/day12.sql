...
coalesce(B.name,'Vacent') as name
from
(select *, GENERATE_SERIES(1,totalpost) as row_number
from job_positions)A
left join 
(select *, row_number() over(partition by position_id order by id) as row_number 
from job_employees)B
on A.id = B.position_id and A.row_number = B.row_number
